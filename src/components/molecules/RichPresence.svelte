<script lang="ts">
	...
	import { user } from '../../utils/discord';
	import type { Spotify } from '../../utils/types';

    import defaultPicture from '../../assets/default.webp?url';

	let activity = `@${user.username}`,
		details = 'Fetching...',
		activityImage = defaultPicture,
		pulse = 30000,
		activityNumber = 0,
		state: string,
		smallImage: string,
	...
	function localTime() {
		state = new Date().toLocaleTimeString('fr-FR', { timeZone: 'Europe/Paris' });
	}
